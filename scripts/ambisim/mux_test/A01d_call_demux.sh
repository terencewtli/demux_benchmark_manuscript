...
LIST=$PROJDIR/ambient/ambisim/mux_test/txt/experiments.txt

# ID=1
ID=${SGE_TASK_ID}
EXP=$(head -${ID} $LIST | tail -1)
INDIR=$PROJDIR/ambient/ambisim/mux_test/$EXP/
cd $INDIR

N_DONORS=${EXP%_*}
DONORS=$PROJDIR/ambient/ambisim/mux_test/txt/${N_DONORS}_donors.txt
...
DONORS=$PROJDIR/ambient/ambisim/mux_test/txt/${N_DONORS}_donors_090.vcf.gz
