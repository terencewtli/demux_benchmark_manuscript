#!/bin/bash
#$ -cwd
#$ -o logs/A01e_scsplit_preprocess_gex.$JOB_ID
#$ -j y
#$ -N A01e_scsplit_preprocess_gex
...
time umi_tools dedup --extract-umi-method tag \
    --umi-separator=':' --umi-tag=UB \
    --stdin=$FILTERED_BAM --stdout=$RMDUP_BAM

###
echo 'sort index'
FINAL_BAM=$INDIR/${SAMPLE}.final.bam
# time samtools view -@ 50 -S -b -q 10 -F 3844 $RMDUP_BAM > $FINAL_BAM
